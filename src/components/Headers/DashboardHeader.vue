		...
		<a-layout-header>
			<a-row type="flex">
				...
				<a-col :span="24" :md="6" class="header-control">
					...
					<a-input-search class="header-search" style="width:200%" :class="searchLoading ? 'loading' : ''" placeholder="Search Mail..." @search="onSearch" :loading='searchLoading'>
				...
				<a-col :span="24" :md="18" class="header-control">
					...
					<div v-if="selectedAddress">
						<a-button type="dashed" shape="round" @click="copyAddress">{{ selectedAddress }}</a-button>
					</div>
					<!-- 未连接钱包 -->
					<div v-if="!selectedAddress">
	...
	export default ({
		...
		methods: {
			...
			_initAccount(userAddress) {
				...
				this.$store.commit("setInboxRefresh", true); // 触发收件箱更新
			...
			_resetAccount() {
				...
				this.$store.commit('resetProvider');
				this.$store.commit("setInboxRefresh", true); // 触发收件箱更新
				this.$store.commit("setSentBoxRefresh", true);
			},
			 ...
			 */
			async copyAddress() {
				await this.$copyText(this.selectedAddress);
				this.$message.success("Address Copid!");
			}
		},
		mounted: function(){
